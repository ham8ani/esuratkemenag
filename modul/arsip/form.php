<?php
	...
	if(isset($_GET['hal']))
	{
		...
		if($_GET['hal'] == "edit")
		{
			...
			$tampil = mysqli_query($koneksi, "SELECT 
    				  ...
    				  WHERE 
					    ...
					    and tbl_suratmasuk.id_arsip='$_GET[id]'");
			...
			if($data)
			{
				...
				$vprihal = $data['prihal'];
				$vid_disposisi = $data['id_disposisi'];
		...
		{
			$hapus = mysqli_query($koneksi, "DELETE FROM tbl_suratmasuk WHERE id_arsip='$_GET[id]'");
			if($hapus){
				echo "<script>
	...
	if(isset($_POST['bsimpan']))
	{
		...
		if(@$_GET['hal'] == "edit"){
			...
			$ubah = mysqli_query($koneksi, "UPDATE tbl_suratmasuk SET 
		...
		else
		{
			...
			//simpan data
			$file = upload();
			$simpan = mysqli_query($koneksi, "INSERT INTO tbl_suratmasuk
											  VALUES (	'', 
											  		  	'$_POST[no_surat]', 
											  		  	'$_POST[tanggal_surat]',
...
<div class="card mt-3">
  ...
  <div class="card-body">
    <form method="post" action="" enctype="multipart/form-data" >
	  ...
	  <div class="form-group">
	    <label for="id_disposisi">Tujuan</label>
	   	<select class="form-control" name="id_disposisi">
	   		<option value="<?=@$vid_disposisi?>"><?=@$vnama_disposisi?></option>
	   		<?php
	   			$tampil = mysqli_query($koneksi, "SELECT * from tbl_disposisi order by nama_disposisi asc");
	   			while($data = mysqli_fetch_array($tampil)){
	   				echo "<option value = '$data[id_disposisi]'> $data[nama_disposisi] </option> ";
	   			}
